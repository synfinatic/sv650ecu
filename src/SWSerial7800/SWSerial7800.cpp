/*
Modified by Aaron Turner to support 7800 baud
...
SWSerial7800::SWSerial7800(uint8_t rxPin, uint8_t txPin, bool inverse_logic /* = false */)
{
...
void SWSerial7800::begin(unsigned long speed)
{
	if (port) {
		port->begin(speed);
	} else {
...
// the transmit waveform.  Increasing this number causes SWSerial7800
...
size_t SWSerial7800::write(uint8_t b)
{
...
int SWSerial7800::available()
{
	if (port) return port->available();
	return 0;
}

int SWSerial7800::peek()
{
	if (port) return port->peek();
	return -1;
}

int SWSerial7800::read()
{
...
#error This version of SWSerial7800 supports only 20, 16 and 8MHz processors
...
// Statics
//
SWSerial7800 *SWSerial7800::active_object = 0;
char SWSerial7800::_receive_buffer[_SS_MAX_RX_BUFF]; 
volatile uint8_t SWSerial7800::_receive_buffer_tail = 0;
volatile uint8_t SWSerial7800::_receive_buffer_head = 0;
...
/* static */ 
inline void SWSerial7800::tunedDelay(uint16_t delay) { 
  uint8_t tmp=0;
...
void SWSerial7800::tx_pin_write(uint8_t pin_state)
{
  if (pin_state == LOW)
...
uint8_t SWSerial7800::rx_pin_read()
{
...
/* static */
inline void SWSerial7800::handle_interrupt()
{
  if (active_object)
  {
    active_object->recv();
...
// Constructor
//
SWSerial7800::SWSerial7800(uint8_t receivePin, uint8_t transmitPin, bool inverse_logic /* = false */) : 
...
// Destructor
//
SWSerial7800::~SWSerial7800()
{
  end();
}

void SWSerial7800::setTX(uint8_t tx)
{
  pinMode(tx, OUTPUT);
...
void SWSerial7800::setRX(uint8_t rx)
{
  pinMode(rx, INPUT);
  if (!_inverse_logic)
...
void SWSerial7800::begin(long speed)
{
